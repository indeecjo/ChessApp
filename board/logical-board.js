...
chessApp.factory('Board',function(Pawn,Piece,Cell,Rook,King,Bishop,Queen,Knight,Move,PhysicalBoard){ 
  ...
  Board.prototype.isCastleLegal = function(move){
    ...
    if(king.castle === King.CASTLE_QUEEN_SIDE){
      if(this.physicalBoard.isEmpty({x:move.to.x,y:move.to.y-1})){
        return false;
      }
    }
    return true;
  ...
  Board.prototype.changeCastleStatus = function(piece){
    if(piece instanceof King){
      ...
      if( !(this.physicalBoard.getPiece({x:x,y:0}) instanceof Rook) ){
        this.canCastleQueenSide[piece.color] = false;
      }
      if( !(this.physicalBoard.getPiece({x:x,y:7}) instanceof Rook) ){
        this.canCastleKingSide[piece.color] = false;
      }
    }
  }
  ...
  Board.prototype.completeCastle = function(piece){    
    ...
    this.physicalBoard.removePiece(oldRookCoord);//boardMatrix[oldRookCoord.x][oldRookCoord.y] = new Cell(oldRookCoord);
    movingRook.move(newRookCoord);
    this.physicalBoard.setPiece(newRookCoord,movingRook)//this.boardMatrix[newRookCoord.x][newRookCoord.y] = new Cell(newRookCoord,movingRook);    
  }
